...
const App = ({ mode }) => {
  ...
  const getWalletInfo = async () => {
    ...
    // const walletAddress = wallet?.properties?.jackalAccount?.address;
    // (Wallet address, duration in hours (min 720),
    // space in bytes (min 1000000000)
    // if (walletAddress) {
    //   const storageData = await storage.getClientFreeSpace(walletAddress);
    //   if (storageData === 0) {
    //     await storage.buyStorage(walletAddress, 720, 1000000000000);
    //   }
    // }
    // const fileIo = await FileIo.trackIo(wallet);
    // await fileIo.refresh();
    // await fileIo.deleteHome();
    // const listOfFolders = ["Data"];
    ...
    // console.log(msg)
    // const initialData = await fileIo.generateInitialDirs(msg, listOfFolders);
    // console.log(initialData?.transactionHash);
    // // // after the first time, this code can be used instead. this will only create new folders if they don't already exist
    const fileIo = await FileIo.trackIo(wallet);

    const parentFolderPath = "Home"; // replace this with your own path
    const fileName = "logo"; // replace this with your own file name
    ...
    const data = await fileIo.staggeredUploadFiles(uploadList, parent, {
      complete: 0,
      timer: 0,
    });
  ...
  const getParams = ({ maxGridSize }) => {
    ...
    const paramKeys = {
      ...
      webcamEnabled: "true",
      name: "",
      meetingId: "ThePodocasts",
      region: "sg001",
      preferredProtocol: "UDP_ONLY",
      canChangeLayout: "true",
      redirectOnLeave: "https://podocast.onrender.com",
      chatEnabled: "true",
      theme: "DEFAULT",
